...
class spec:
  ...
  @staticmethod
  def create_timestamp_mask(ts, start, end):
    ...
    return ((ts >= start_seconds) & (ts <= end_seconds))
  ...
  @staticmethod
  def barrel_sp_collect_spectra(ss, altitude=None, maglat=None, bkg_method=1):
    if altitude is None:
      pyspedas.barrel.ephm(ss["trange"][0], ss["payload"])
      ts, alt = pytplot.get_data('brl'+ss["payload"]+'_GPS_Alt')
      ss["altitude"] = altitude if altitude or alt[int(alt.size/2)]
      print(ss["altitude"])

    if maglat is None and bkg_method==2:
      print("Please supply magnetic latitude")
      return
    
    width, mean, gmean = spec.edge_products(ss["e_bins"]) 
    
    for (start, end) in ss["event_periods"]:
  ...
  def barrel_sp_collect_one_spectrum(ss, start, end):
    ...
    ebins=spec.make_standard_energies(True)
    sspc_ts, sspc_cnts, energy_levels = pytplot.get_data(ss["tplot_var"])
    spectots = raw_sspc.sum(1)
    goodspect = spectots >= 0.1

    pyspedas.barrel.rcnt(start, end, ss["payload"])
    rcnt_ts, rcnt_cnts = pytplot.get_data("brl_"+ss["payload"]+"_RCNT_Interrupt")
    badirq = rcnt_cnts < 0
